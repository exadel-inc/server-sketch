...
function installTranspileHooks(scopeRoot) {
  ...
  const installedExts = [];
  const loadedProjectFiles = new Set();

  const rollbackHandler = (ext) => {
    if (require.extensions[ext] === installedHandlers[ext]) {
  ...
  try {
    JSX_EXTENSIONS.forEach((ext) => {
      ...
      const handler = createTranspileHandler({
        ext,
        scopeRoot,
        nodeModulesMarker,
        previousLoader: previous[ext],
        loadedProjectFiles
      });
      installedHandlers[ext] = handler;
      require.extensions[ext] = handler;
      installedExts.push(ext);
    });
  } catch (error) {
    // Roll back only handlers installed by this invocation.
    installedExts.forEach(rollbackHandler);
    throw error;
  }

  return () => {
    JSX_EXTENSIONS.forEach(rollbackHandler);
    loadedProjectFiles.forEach((file) => delete require.cache[file]);
  };
}

function createTranspileHandler({ ext, scopeRoot, nodeModulesMarker, previousLoader, loadedProjectFiles }) {
  return (mod, file) => {
    const relativeFile = path.relative(scopeRoot, file);
    const isProjectFile =
      relativeFile !== '' &&
      relativeFile !== '..' &&
      !relativeFile.startsWith(`..${path.sep}`) &&
      !path.isAbsolute(relativeFile) &&
      !file.includes(nodeModulesMarker);

    if (isProjectFile) {
      loadedProjectFiles.add(file);
      mod.paths = [...Module._nodeModulePaths(path.dirname(file)), ...module.paths];
      mod._compile(transpile(file), file);
    } else if (previousLoader) {
      previousLoader(mod, file);
    } else {
      throw new Error(
