...
class Component {
  ...
  _init() {
    try {
      ...
      this._initView(configJSON);
      this._loadController(configJSON);
  ...
  // Prepares the component view. Base implementation reads the view file as a
  // doT.js HTML template; override it in subclasses (e.g. JsxComponent).
  _initView(configJSON) {
    this.viewFile = configJSON.view;
    this.view = this._getFile(configJSON.view);
  }

  // Extends the component instance with the controller object and calls its init().
  _loadController(configJSON) {
  ...
  initComponent(alias, options) {
    return require('./component-factory').init(alias, options, this);
  }
