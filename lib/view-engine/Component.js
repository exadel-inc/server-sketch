...
class Component {
   ...
   * pointing `config.json > view` at a `.jsx`/`.tsx` file. Everything else
   * renders with doT.js. Because this lives on the base class, subclasses (e.g.
   * projects that set `config.componentClass` to their own Component) inherit
   * JSX support automatically — no factory or separate component class needed.
   */
  _getRendererName() {
    const componentConfig = this.config || {};
    const configured = componentConfig.renderer && String(componentConfig.renderer).trim().toLowerCase();

    // Unknown renderer values never reach require() — they're treated as
    // "not set". Warn once per instance so config typos are easy to spot.
    if (configured && !RENDERERS.includes(configured) && !this._unknownRendererWarned) {
      this._unknownRendererWarned = true;
      console.warn(`[server-sketch] Unknown renderer "${componentConfig.renderer}" in "${this.name}" config. ` +
        `Allowed: ${RENDERERS.join(', ')}. Falling back to the default renderer.`);
    }

    if (configured && RENDERERS.includes(configured)) {
      return configured;
    }

    // React views (.jsx / .tsx) are handled by the "jsx" renderer.
    return ['.jsx', '.tsx'].includes(path.extname(componentConfig.view || '').toLowerCase()) ? 'jsx' : 'dot-js';
  }
